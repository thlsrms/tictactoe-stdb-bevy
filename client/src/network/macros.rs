/// Helper macro to register callbacks and subscriptions
#[cfg(not(target_arch = "wasm32"))]
macro_rules! stdb_subscribe {
    // Match update handlers (ctx, old, new)
    ($app:ident, $conn:expr, $evt_ty:ident, $table:ident) => {
        let (tx, rx) = crossbeam_channel::unbounded::<$evt_ty<$table>>();
        paste::paste! {
            $conn.db.[<$table:snake>]().[<$evt_ty:snake>](move |_ctx, old, new| {
                tx.send(OnUpdate { old: old.clone(), new: new.clone() }).unwrap();
            });
        }

        stdb_register_event!($app, OnUpdate<$table>);
        ($app).insert_resource(EventQueue::<OnUpdate<$table>>(Mutex::new(rx)));
    };

    // Match insert/delete/etc. handlers (ctx, row)
    ($app:ident, $conn:expr, $evt_ty:ident<$table:ty>) => {
        let (tx, rx) = crossbeam_channel::unbounded::<$evt_ty<$table>>();
        paste::paste! {
            $conn.db.[<$table:snake>]().[<$evt_ty:snake>](move |_ctx, row: &$table| {
                tx.send($evt_ty(row.clone())).unwrap();
            });
        }

        stdb_register_event!($app, $evt_ty<$table>);
        ($app).insert_resource(EventQueue::<$evt_ty<$table>>(Mutex::new(rx)));
    };

    // Match subscription applied (1 argument: ctx)
    ($app:ident, $conn:expr, $sub_query:expr, $evt_ty:ty, $wrap:expr) => {{
        let (tx, rx) = crossbeam_channel::unbounded::<$evt_ty>();
        $conn
            .subscription_builder()
            .on_applied(move |ctx| {
                tx.send($wrap(ctx)).unwrap();
            })
            .subscribe($sub_query);

        stdb_register_event!($app, $evt_ty);
        ($app).insert_resource(EventQueue::<$evt_ty>(Mutex::new(rx)));
    }};

    // Match subscription applied and error (1 argument: ctx)
    ($app:ident, $conn:expr, $sub_query:expr, $evt_ty:ty, $wrap:expr, $err_ty:ty, $wrap_err:expr) => {{
        let (tx, rx) = crossbeam_channel::unbounded::<$evt_ty>();
        let (tx_err, rx_err) = crossbeam_channel::unbounded::<$err_ty>();
        $conn
            .subscription_builder()
            .on_applied(move |ctx| {
                tx.clone().send($wrap(ctx)).unwrap();
            })
            .on_error(move |ctx, err| {
                tx_err.send($wrap_err(ctx, err)).unwrap();
            })
            .subscribe($sub_query);

        stdb_register_event!($app, $evt_ty);
        ($app).insert_resource(EventQueue::<$evt_ty>(Mutex::new(rx)));

        stdb_register_event!($app, $err_ty);
        ($app).insert_resource(EventQueue::<$err_ty>(Mutex::new(rx_err)));
    }};
}

/// Register the events for the lifecycle callbacks: OnConnect, OnConnectError, OnDisconnect
macro_rules! stdb_lifecycle {
    ($app:ident, $conn_builder:ident, ($conn_evt_ty:ty, $conn_err_evt_ty:ty, $disconn_evt_ty:ty)) => {{
        let (conn_tx, conn_rx) = crossbeam_channel::unbounded::<$conn_evt_ty>();
        stdb_register_event!($app, $conn_evt_ty);
        ($app).insert_resource(EventQueue::<$conn_evt_ty>(Mutex::new(conn_rx)));

        let (conn_err_tx, conn_err_rx) = crossbeam_channel::unbounded::<$conn_err_evt_ty>();
        stdb_register_event!($app, $conn_err_evt_ty);
        ($app).insert_resource(EventQueue::<$conn_err_evt_ty>(Mutex::new(conn_err_rx)));

        let (disconn_tx, disconn_rx) = crossbeam_channel::unbounded::<$disconn_evt_ty>();
        stdb_register_event!($app, $disconn_evt_ty);
        ($app).insert_resource(EventQueue::<$disconn_evt_ty>(Mutex::new(disconn_rx)));

        $conn_builder
            .on_connect(move |_ctx, identity, token: &str| {
                conn_tx.send(<$conn_evt_ty>::new(identity, token)).unwrap();
            })
            .on_connect_error(move |_ctx, err| {
                conn_err_tx.send(<$conn_err_evt_ty>::new(err)).unwrap();
            })
            .on_disconnect(move |_ctx, err| {
                disconn_tx.send(<$disconn_evt_ty>::new(err)).unwrap();
            })
    }};
}

macro_rules! stdb_register_event {
    ($app:ident, $( $evt_ty:ty ),* ) => {
        $(
            ($app).add_event::<NetworkEvent<$evt_ty>>();
            ($app).add_systems(bevy::prelude::PreUpdate, process_network_queue::<$evt_ty>);
        )*
    }
}

// For wasm we need to split the register of the event from the subcription to the events because
// we poll before connecting and we wouldn't be able to register the event and system anymore.
#[cfg(target_arch = "wasm32")]
macro_rules! stdb_subscribe {
    // Match update handlers (ctx, old, new)
    ($cmds:ident, $conn:expr, $evt_ty:ident, $table:ident) => {
        let (tx, rx) = crossbeam_channel::unbounded::<$evt_ty<$table>>();
        paste::paste! {
            $conn.db.[<$table:snake>]().[<$evt_ty:snake>](move |_ctx, old, new| {
                tx.send(OnUpdate { old: old.clone(), new: new.clone() }).unwrap();
            });
        }
        ($cmds).insert_resource(EventQueue::<OnUpdate<$table>>(Mutex::new(rx)));
    };

    // Match insert/delete/etc. handlers (ctx, row)
    ($cmds:ident, $conn:expr, $evt_ty:ident<$table:ty>) => {
        let (tx, rx) = crossbeam_channel::unbounded::<$evt_ty<$table>>();
        paste::paste! {
            $conn.db.[<$table:snake>]().[<$evt_ty:snake>](move |_ctx, row: &$table| {
                tx.send($evt_ty(row.clone())).unwrap();
            });
        }
        ($cmds).insert_resource(EventQueue::<$evt_ty<$table>>(Mutex::new(rx)));
    };

    // Match subscription applied (1 argument: ctx)
    ($cmds:ident, $conn:expr, $sub_query:expr, $evt_ty:ty, $wrap:expr) => {{
        let (tx, rx) = crossbeam_channel::unbounded::<$evt_ty>();
        $conn
            .subscription_builder()
            .on_applied(move |ctx| {
                tx.send($wrap(ctx)).unwrap();
            })
            .subscribe($sub_query);
        ($cmds).insert_resource(EventQueue::<$evt_ty>(Mutex::new(rx)));
    }};

    // Match subscription applied and error (1 argument: ctx)
    ($cmds:ident, $conn:expr, $sub_query:expr, $evt_ty:ty, $wrap:expr, $err_ty:ty, $wrap_err:expr) => {{
        let (tx, rx) = crossbeam_channel::unbounded::<$evt_ty>();
        let (tx_err, rx_err) = crossbeam_channel::unbounded::<$err_ty>();
        $conn
            .subscription_builder()
            .on_applied(move |ctx| {
                tx.clone().send($wrap(ctx)).unwrap();
            })
            .on_error(move |ctx, err| {
                tx_err.send($wrap_err(ctx, err)).unwrap();
            })
            .subscribe($sub_query);
        ($cmds).insert_resource(EventQueue::<$evt_ty>(Mutex::new(rx)));
        ($cmds).insert_resource(EventQueue::<$err_ty>(Mutex::new(rx_err)));
    }};
}
